--Project name: Library Management System (Project 2)
...
-- while importing data value error occurred, let's fix it with ALTER 
...
-- (ERROR:insert or update on table "return_status" violates foreign key constraint "fk_issued_status" | Key (issued_id)=(IS101) is not present in table "issued_status". - fixed that)
--deleting RS101 up to RS105 as it not available in the issued table
...
-- double-check tables for correct data 
...
-- task 11: Create a Table of Books with Rental Price Above a Certain Threshold (6.5).
...
SELECT
     --(name of the book) DISTINCT ist.issued_book_name
...
-- error on procedure
-- fixing the problem on 'book_title'
